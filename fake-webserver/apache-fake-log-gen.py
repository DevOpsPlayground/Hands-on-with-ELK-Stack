...
parser.add_argument("--start-date", "-s", dest='start_date', help="Date to start generating logs from. Must be in format dd/mm/yyyy", type=str)
parser.add_argument("--slow-down", "-d", dest='slow_down', help="Slow down log generation to avoid overload cpu.")
...
if args.start_date:
    otime = datetime.datetime.strptime(args.start_date, "%d/%m/%Y")
else:
...
flag = True
while (flag):
	increment = datetime.timedelta(seconds=random.randint(1,10))
	otime += increment

	ip = faker.ipv4()
	dt = datetime.datetime.utcnow().strftime('%d/%b/%Y:%H:%M:%S')
	tz = datetime.datetime.now(pytz.timezone('Europe/London')).strftime('%z')
